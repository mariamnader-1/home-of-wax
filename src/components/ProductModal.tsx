...
export default function ProductModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(10,7,3,0.88)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, backdropFilter:'blur(8px)', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#F8F4EE', borderRadius:18, maxWidth:520, width:'100%', maxHeight:'90vh', overflowY:'auto', position:'relative', boxShadow:'0 30px 90px rgba(0,0,0,0.45)', animation:'modalIn 0.3s ease' }}>

        {/* Modal Image */}
        <div style={{ width:'100%', height:240, overflow:'hidden', borderRadius:'18px 18px 0 0' }}>
          <img src={product.image} alt={product.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} />
        </div>

        <div style={{ padding:40 }}>
          <button onClick={onClose} style={{ position:'absolute', top:18, right:22, background:'rgba(255,255,255,0.9)', border:'none', borderRadius:'50%', width:32, height:32, fontSize:16, cursor:'pointer', color:'#8a7a6a', display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
